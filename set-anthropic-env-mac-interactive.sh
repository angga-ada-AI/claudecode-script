...
if [[ "$SAVE_PERMANENT" =~ ^[Yy]$ ]]; then
    if [ -z "$SHELL_CONFIG" ]; then
        echo "Error: Tidak bisa menentukan file konfigurasi shell Anda."
        exit 1
    fi
    ...
    if grep -q "ANTHROPIC_BASE_URL" "$SHELL_CONFIG" 2>/dev/null; then
        echo "Warning: Environment variables sudah ada di $SHELL_CONFIG"
    ...
    echo ""
    echo "Info: Environment variables hanya berlaku untuk terminal saat ini."
