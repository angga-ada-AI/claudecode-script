...
1. Purpose & big picture
   ...
   - Core integrations: the CLI/extension communicate with the Anthropic API at `https://api.minimax.io/anthropic` using `ANTHROPIC_AUTH_TOKEN` and `ANTHROPIC_BASE_URL` environment variables.
...
4. Windows-specific conventions & gotchas (very important)
   - Environment variables (Windows): `setx ANTHROPIC_AUTH_TOKEN your_api_key` and `setx ANTHROPIC_BASE_URL https://api.minimax.io/anthropic` (restart terminal/VS Code after `setx`).
